...
TaipeiTime = UtcTime+timedelta
nowdatestr = TaipeiTime.strftime("%Y/%m/%d")
# print(nowdatestr)
...
for i, sheet in enumerate(sheetList):
    datelist = sheet.findall(re.compile('^'+ nowdatestr), None, 2)

    if(datelist!=[]):
        firstrow = datelist[0].row
        lastrow = datelist[-1].row
        logging.info("%s date before %s , is delete first row=%d, last row=%d" % (sheet.title,nowdatestr, firstrow, lastrow))
        sheet.delete_rows(2,lastrow)
    else:
        logging.info("no data in %s be delete"% sheet.title )


# sometime=datetime.datetime.strptime('2021/07/07 00:00:02',"%Y/%m/%d %H:%M:%S")
# subtime=datetime.datetime.now()-sometime
